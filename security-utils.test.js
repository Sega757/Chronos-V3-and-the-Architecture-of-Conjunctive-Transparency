...
tests['isPrivateIp should return false for invalid types or invalid IP formats'] = () => {
  assert.strictEqual(isPrivateIp(null), false);
  assert.strictEqual(isPrivateIp(undefined), false);
  assert.strictEqual(isPrivateIp(12345), false);
  ...
  assert.strictEqual(isPrivateIp('::g1'), false);
};

tests['isPrivateIp should return false for valid public IPv4 addresses'] = () => {
  assert.strictEqual(isPrivateIp('8.8.8.8'), false);
  assert.strictEqual(isPrivateIp('1.1.1.1'), false);
  assert.strictEqual(isPrivateIp('142.250.190.46'), false);
};

tests['isPrivateIp should return true for private, loopback, link-local, and reserved IPv4 addresses'] = () => {
...
tests['isPrivateIp should correctly handle surrounding square brackets on IPv6 addresses'] = () => {
  assert.strictEqual(isPrivateIp('[::1]'), true);
  assert.strictEqual(isPrivateIp('[2606:4700:4700::1111]'), false);
  assert.strictEqual(isPrivateIp('[fe80::1]'), true);
};

tests['isPrivateIp should correctly recognize IPv4-mapped and compatible IPv6 addresses'] = () => {
...
// --- Tests for safeParseUrl option blockPrivateIps ---
tests['safeParseUrl with blockPrivateIps should allow public domains and public IPs'] = () => {
  const url1 = safeParseUrl('https://example.com/path', null, { blockPrivateIps: true });
  assert.notStrictEqual(url1, null);
  assert.strictEqual(url1.hostname, 'example.com');

  const url2 = safeParseUrl('http://8.8.8.8/api', null, { blockPrivateIps: true });
  assert.notStrictEqual(url2, null);
  assert.strictEqual(url2.hostname, '8.8.8.8');
};

tests['safeParseUrl with blockPrivateIps should reject localhost and loopback names'] = () => {
  assert.strictEqual(safeParseUrl('http://localhost', null, { blockPrivateIps: true }), null);
  assert.strictEqual(safeParseUrl('http://localhost:3000', null, { blockPrivateIps: true }), null);
  assert.strictEqual(safeParseUrl('http://sub.localhost:8080/test', null, { blockPrivateIps: true }), null);
};

tests['safeParseUrl with blockPrivateIps should reject private, loopback, and local IP addresses'] = () => {
  assert.strictEqual(safeParseUrl('http://127.0.0.1/api', null, { blockPrivateIps: true }), null);
  assert.strictEqual(safeParseUrl('http://192.168.1.1/admin', null, { blockPrivateIps: true }), null);
  assert.strictEqual(safeParseUrl('http://10.0.0.1:8080/', null, { blockPrivateIps: true }), null);
  assert.strictEqual(safeParseUrl('http://[::1]/', null, { blockPrivateIps: true }), null);
  assert.strictEqual(safeParseUrl('http://[fe80::1]/', null, { blockPrivateIps: true }), null);
  assert.strictEqual(safeParseUrl('http://[::ffff:127.0.0.1]/', null, { blockPrivateIps: true }), null);
};

// --- Tests for trailing dot SSRF/Open Redirect bypass mitigations ---
tests['safeParseUrl with blockPrivateIps should reject loopback names with trailing dots'] = () => {
  assert.strictEqual(safeParseUrl('http://localhost.', null, { blockPrivateIps: true }), null);
  assert.strictEqual(safeParseUrl('http://localhost.:3000', null, { blockPrivateIps: true }), null);
  assert.strictEqual(safeParseUrl('http://sub.localhost.:8080/test', null, { blockPrivateIps: true }), null);
};

tests['safeParseUrl with allowedHosts should match and allow allowed domains with trailing dots'] = () => {
  const allowed = ['example.com', '.trusted.org'];

  const url1 = safeParseUrl('https://example.com.', allowed);
  assert.notStrictEqual(url1, null);
  assert.strictEqual(url1.hostname, 'example.com.');

  const url2 = safeParseUrl('https://sub.trusted.org.', allowed);
  assert.notStrictEqual(url2, null);
  assert.strictEqual(url2.hostname, 'sub.trusted.org.');
};

tests['isPrivateIp should correctly recognize IP addresses with trailing dots'] = () => {
  assert.strictEqual(isPrivateIp('127.0.0.1.'), true);
  assert.strictEqual(isPrivateIp('10.0.0.1.'), true);
  assert.strictEqual(isPrivateIp('8.8.8.8.'), false);
  assert.strictEqual(isPrivateIp('::1.'), true);
